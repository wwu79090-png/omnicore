...
export class AITilemapGenerator {
  static generate(prompt, {
    ...
    if (/forest|森林/u.test(text)) fill(data, width, height, (x, y) => x < Math.ceil(width * 0.35) || y < 2, TILE.forest);
    if (/lake|water|湖|河/u.test(text)) {
      const cx = Math.floor(width / 2);
      const cy = Math.floor(height / 2);
      fill(data, width, height, (x, y) => Math.abs(x - cx) + Math.abs(y - cy) <= 2, TILE.water);
    }
    if (/cliff|悬崖|峭壁/u.test(text)) fill(data, width, height, (x) => x >= Math.floor(width * 0.75), TILE.cliff);
    return {
      tilemapJson: {
        width,
        height,
